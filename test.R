...
### display the pdf ########################################################
  observe({
    ...
    file.copy(input$file_import$datapath,"www", overwrite = T)
...
### OCR ####################################################################
  pngfile1 <- reactive({pdftools::pdf_convert(input$file_import$datapath,
                                              dpi = 300)})
...
### To be deleted #############################################################  
  ...
  observeEvent(input$file_import,{
    df2 = reactive({
      tibble(
        pdf_name = input$file_import$name,
...
###############################################################################
  ...
  observeEvent(input$approve, {
    ...
    file.rename(input$file_import$datapath,
                df3()[["pdf_name"]]
                )
